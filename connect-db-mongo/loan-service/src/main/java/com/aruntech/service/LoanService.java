...
public class LoanService {
    ...
    @Autowired
    private LoanSeqGenerator seqGenerator;
    @Autowired
    ...
    public LoanResponse applyLoan(final LoanRequest request) {
        ...
        Loan loan = new Loan();
        loan.setId(seqGenerator.getSequence(Loan.LOAN_SEQ));
    ...
    private boolean checkCustomerDetails(final LoanRequest request) {
        Customer customer = restTemplate.getForObject(CUSTOMER_ENDPOINT_URL+"/"+request.getCustId(), Customer.class);
        if(customer == null) {
